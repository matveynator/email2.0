	...
	if cmd.params[1] != "<>" {
		...
		if (session.peer.Authenticated!=true) {
			...
			if err!=nil {
				...
				if valid!=true {
					session.reply(502, "SPF Invalid. Please auth or fix your SPF DNS record.")
					session.close()
					return
				}
			}
		...
		if (session.peer.Authenticated!=true) {
			var valid bool
			valid,err=session.CheckIPIsAllowed(session.peer.Addr.(*net.TCPAddr).IP.String(), session.peer.HeloName) 
			if err!=nil {
				session.reply(451, "Temporary server DNS error. Please try again later. Error: " + err.Error())
				session.close()
				return
...
func (session *session) handleRCPT(cmd command) {
	...
	//Allow authenticated users to send mail anywhere, but leave unauthenticated users to send mail only to domains for which we are MX and SPF.

	if (session.peer.Authenticated!=true) {
		var valid bool
		valid, err = session.CheckRelayIsAllowed(domain)
		if err!=nil {
			session.reply(451, "Temporary server DNS error. Please try again later. Error: " + err.Error())
			session.close()
			return
			...
			if valid!=true {
				session.reply(571, "Sorry. We do not relay mail for " + domain + ".")
				session.close()
				return
			}
		}
...
func (session *session) handleDATA(cmd command) {
	...
	if err == io.EOF {
		...
		//check bug here CheckIPIsAllowed "FROM" and "Reply-To" along with "Return-Path" #1

		reader := strings.NewReader(string(data.Bytes()))
		mailData, err := mail.ReadMessage(reader)
		if err != nil {
			fmt.Println(err)
		}
		...
		fromAddr, err := parseAddress(mailHeader.Get("From"))
		if err != nil {
